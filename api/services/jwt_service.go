...
type IJWTService interface {
	CreateJWTToken(c echo.Context, objectId bson.ObjectId, username string, email string, firstName string, lastName string) error
...
func (self *JWTService) CreateJWTToken(c echo.Context, objectId bson.ObjectId, username string, email string, firstName string, lastName string) error {
	token := jwt.New(jwt.SigningMethodHS256)

	// Set claims
	claims := token.Claims.(jwt.MapClaims)
	claims["id"] = objectId.Hex()
