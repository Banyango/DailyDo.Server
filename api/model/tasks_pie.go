...
func (ss Tasks) All(fn func(value Task) bool) bool {
...
func (ss Tasks) Any(fn func(value Task) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss Tasks) Append(elements ...Task) Tasks {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(Tasks{}, ss...)
...
func (ss Tasks) Bottom(n int) (top Tasks) {
...
func (ss Tasks) Contains(lookingFor Task) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss Tasks) Diff(against Tasks) (added, removed Tasks) {
	...
	diffOneWay := func(ss1, ss2raw Tasks) (result Tasks) {
		ss2 := make(Tasks, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss Tasks) DropTop(n int) (drop Tasks) {
	if n < 0 || n >= len(ss) {
		return
	}
...
//   })
//
func (ss Tasks) Each(fn func(Task)) Tasks {
	for _, s := range ss {
		fn(s)
	}
...
func (ss Tasks) Equals(rhs Tasks) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss Tasks) Extend(slices ...Tasks) (ss2 Tasks) {
	ss2 = ss
...
func (ss Tasks) Filter(condition func(Task) bool) (ss2 Tasks) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Tasks) FilterNot(condition func(Task) bool) (ss2 Tasks) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Tasks) FindFirstUsing(fn func(value Task) bool) int {
...
func (ss Tasks) First() Task {
	return ss.FirstOr(Task{})
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss Tasks) FirstOr(defaultValue Task) Task {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Tasks) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss Tasks) Insert(index int, values ...Task) Tasks {
	if index >= ss.Len() {
		return Tasks.Extend(ss, Tasks(values))
	}

	return Tasks.Extend(ss[:index], Tasks(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss Tasks) Ints() pie.Ints {
	l := len(ss)
...
func (ss Tasks) Join(glue string) (s string) {
	var slice interface{} = []Task(ss)
...
func (ss Tasks) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss Tasks) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss Tasks) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss Tasks) Last() Task {
	return ss.LastOr(Task{})
}

// LastOr returns the last element or a default value if there are no elements.
func (ss Tasks) LastOr(defaultValue Task) Task {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Tasks) Map(fn func(Task) Task) (ss2 Tasks) {
	if ss == nil {
		return nil
	}

	ss2 = make([]Task, len(ss))
	for i, s := range ss {
		ss2[i] = fn(s)
	}
...
func (ss Tasks) Mode() Tasks {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[Task]int)
	for _, s := range ss {
		values[s]++
	}
...
func (ss Tasks) Random(source rand.Source) Task {
	n := len(ss)
...
func (ss Tasks) Send(ctx context.Context, ch chan<- Task) Tasks {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss Tasks) SequenceUsing(creator func(int) Task, params ...int) Tasks {
	var seq = func(min, max, step int) (seq Tasks) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(Tasks, lenght)
...
func (ss Tasks) Shift() (Task, Tasks) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss Tasks) Shuffle(source rand.Source) Tasks {
	n := len(ss)
...
func (ss Tasks) SortStableUsing(less func(a, b Task) bool) Tasks {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
// slice returned will be reallocated as to not modify the input slice.
func (ss Tasks) SortUsing(less func(a, b Task) bool) Tasks {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//   fmt.Sprintf("%v")
//
func (ss Tasks) Strings() pie.Strings {
	l := len(ss)
...
func (ss Tasks) SubSlice(start int, end int) (subSlice Tasks) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]Task, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]Task, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss Tasks) Top(n int) (top Tasks) {
...
func (ss Tasks) StringsUsing(transform func(Task) string) pie.Strings {
	l := len(ss)
...
func (ss Tasks) Unshift(elements ...Task) (unshift Tasks) {
	unshift = append(Tasks{}, elements...)
